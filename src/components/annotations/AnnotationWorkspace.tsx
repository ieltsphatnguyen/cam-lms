...
export default function AnnotationWorkspace({
  ...
  const [criterionScores, setCriterionScores] = useState<Record<number, number | null>>({});
  const [scoreSaving, setScoreSaving] = useState<Record<number, boolean>>({});
  ...
  const [revisionToast, setRevisionToast] = useState(false);
  ...
  const overallBand = computeOverallBand(criteria.map((c) => criterionScores[c.id] ?? null));

  const handleScoreChange = useCallback(
    async (criterionId: number, value: string) => {
      const parsed = value.trim() === '' ? null : parseFloat(value);
      if (parsed !== null && (isNaN(parsed) || parsed < 0 || parsed > 9)) return;

      setCriterionScores((prev) => ({ ...prev, [criterionId]: parsed }));
      setScoreSaving((prev) => ({ ...prev, [criterionId]: true }));
      markDirty();
      try {
        await saveCriterionScore(attempt.id, criterionId, parsed);
      } catch (err) {
        const msg = reportRpcError('save_criterion_score', err);
        alert(`Failed to save score:\n${msg}`);
      } finally {
        setScoreSaving((prev) => ({ ...prev, [criterionId]: false }));
      }
    },
    [attempt.id, markDirty],
  );

  const handleRequestRevision = useCallback(async () => {
    setRequestingRevision(true);
    try {
      await requestRevision(attempt.id);
      setPublished(false);
      setDirty(false);
      setRevisionToast(true);
      setTimeout(() => setRevisionToast(false), 2000);
    } catch (err) {
      const msg = reportRpcError('request_revision', err);
      setErrorToast(`Failed to request revision: ${msg}`);
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex w-[65%] flex-col overflow-y-auto bg-slate-50">
        ...
        <div className="border-t border-slate-200 bg-white p-4">
          <div className="mb-3 flex items-center gap-2">
            <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">Criterion Scores</span>
          </div>
          <div className="space-y-2">
            {criteria.map((criterion) => (
              <div key={criterion.id} className="flex items-center justify-between gap-3 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2">
                <span className="text-sm font-medium text-slate-700">{criterion.name}</span>
                <div className="flex items-center gap-2">
                  {scoreSaving[criterion.id] && (
                  ...
                  <input
                    ...
                    placeholder="—"
                    className="w-16 rounded-md border border-slate-200 bg-white px-2 py-1 text-center text-sm font-semibold text-slate-800 outline-none focus:ring-2 focus:ring-blue-100"
                  />
                </div>
              </div>
            ))}
          ...
          {/* Overall Band */}
          <div className="mt-3 flex items-center justify-between rounded-lg border-2 border-blue-200 bg-blue-50 px-4 py-3">
            <span className="text-sm font-bold uppercase tracking-wide text-blue-700">Overall Band</span>
            <span className={`text-2xl font-bold ${overallBand !== null ? 'text-blue-700' : 'text-slate-300'}`}>
        ...
        <div className="sticky bottom-0 flex items-center justify-between gap-2 border-t border-slate-200 bg-white px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="ghost"
              size="sm"
              icon={requestingRevision ? <Loader2 size={14} className="animate-spin" /> : <RotateCcw size={14} />}
      ...
      {/* Revision Requested Toast */}
      {revisionToast && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center pointer-events-none">
          <div className="flex items-center gap-2 rounded-xl bg-amber-600 px-6 py-3 text-white shadow-2xl">
            <RotateCcw size={18} />
            <span className="text-sm font-semibold">Revision Requested — Student Notified</span>
          </div>
        </div>
      )}
      ...
      {errorToast && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center pointer-events-none">
          <div className="flex items-center gap-2 rounded-xl bg-red-600 px-6 py-3 text-white shadow-2xl">
