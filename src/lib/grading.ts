...
// ── Fetch all attempts for a student+item (submission history) ──
...
export async function fetchSubmissionHistory(
  ...
  const { data: attempts, error } = await supabase
    .from('student_attempts')
    .select('id, status, submitted_at, created_at, feedback_published, revision_requested, revision_notes, response_type, written_response, audio_path')
  ...
  const attemptIds = attempts.map((a) => a.id as number);
  const { data: gradingData } = await supabase
    .from('grading')
    .select('submission_id, grading_status, overall_band_score')
    .in('submission_id', attemptIds);

  const gradingMap = new Map<number, { status: string; band: number | null }>();
  for (const g of gradingData ?? []) {
    gradingMap.set(g.submission_id as number, {
      status: g.grading_status as string,
      band: g.overall_band_score as number | null,
    });
  }

  return (attempts as unknown as SubmissionHistoryEntry[]).map((a) => ({
    ...a,
    overall_band_score: gradingMap.get(a.id)?.band ?? null,
    grading_status: gradingMap.get(a.id)?.status ?? null,
  }));
}
...
export async function getAudioUrl(audioPath: string): Promise<string | null> {
  ...
  if (!url) {
    console.error('[getAudioUrl] getPublicUrl failed for path:', audioPath);
    return null;
  }
  return url;
}
