...
export async function fetchAttemptForItem(
  ...
  const { data, error } = await supabase
    .from('student_attempts')
    .select('id,published_assignment_item_id,student_profile_id,status,started_at,submitted_at,time_limit_seconds,response_type,written_response,audio_path,word_count,created_at,feedback_published')
