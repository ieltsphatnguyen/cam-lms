...
export default function TeacherAssignmentTemplatesPage() {
  ...
  async function handleArchive(t: AssignmentTemplate) {
    try {
      ...
      setError('Failed to archive preset.');
    }
  }
  ...
  async function handleRestore(t: AssignmentTemplate) {
    try {
      ...
      setError('Failed to restore preset.');
    }
  }
  ...
  async function handleEdit(t: AssignmentTemplate) {
    try {
      ...
      setError('Failed to load preset for editing.');
    }
  }
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">
              {view === 'create' ? 'New Assignment Preset' : 'Edit Preset'}
            </h1>
            <p className="mt-1 text-sm text-slate-500">
              {view === 'create'
                ? 'Build a reusable assignment preset from Question Bank questions.'
                : isOwner(editingTemplate as AssignmentTemplate)
                  ? 'Update your preset.'
                  : 'Viewing — you can duplicate this preset.'}
            </p>
          </div>
          <Button
            variant="ghost"
    ...
    <div className="p-6 md:p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Reusable assignment presets built from Question Bank questions.
          </p>
        </div>
        <Button
          icon={<Plus size={16} />}
