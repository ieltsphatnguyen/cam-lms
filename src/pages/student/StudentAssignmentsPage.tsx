...
import { ASSIGNMENT_STATUS_CONFIG } from '@/lib/annotations';
import type {
    ...
    <div className="p-6 md:p-8">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {assignments.map((a) => {
            const status = (a.assignment_status ?? 'not_started') as AssignmentStatus;
            const config = ASSIGNMENT_STATUS_CONFIG[status];
            return (
              <button
                key={a.id}
                onClick={() => onOpenAssignment(a.id)}
                className="flex flex-col rounded-2xl border border-slate-100 bg-white p-5 text-left shadow-sm transition hover:border-blue-200 hover:shadow-md"
              >
                <div className="mb-3 flex items-start justify-between gap-2">
                  ...
                  <span
                    className={`flex shrink-0 items-center gap-1.5 rounded-md px-2 py-0.5 text-xs font-medium ${config.badgeClass}`}
                  >
