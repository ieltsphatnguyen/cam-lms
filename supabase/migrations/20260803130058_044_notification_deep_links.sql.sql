-- Phase 3: Notification deep links with context
-- Teacher notifications: /teacher-grading?attemptId=XXX
-- Student notifications: /student-assignment-detail?assignmentId=XXX

-- ══════════════════════════════════════════════════════════════
-- 1. notify_teacher_of_submission — add attemptId to link
-- ══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_is_resubmission THEN
    ...
    VALUES (
      ...
      '/teacher-grading?attemptId=' || p_attempt_id
    );
  ELSE
    INSERT INTO notifications (recipient_id, sender_id, type, title, body, link)
    VALUES (
...
-- ══════════════════════════════════════════════════════════════
-- 2. request_revision — add assignmentId to link
-- ══════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════
-- 3. publish_feedback — add assignmentId to link
-- ══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT pai.published_assignment_id, pa.name
  INTO v_assignment_id, v_assignment_name
  FROM published_assignment_items pai
  JOIN published_assignments pa ON pa.id = pai.published_assignment_id
  WHERE pai.id = v_item_id;
