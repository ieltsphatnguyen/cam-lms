/*
# Allow Revision Resubmission

## Changes:
1. Updates `start_attempt` RPC so that when a student's existing attempt
   has `revision_requested = true`, the old attempt is kept for reference
   but a NEW in-progress attempt is created, allowing the student to
   resubmit their work.
2. The new attempt inherits `time_limit_seconds` and `response_type` from
   the published item, same as a fresh attempt.
3. Non-revision submitted attempts still block new attempts (one attempt
   per item unless revision is requested).

## Important Notes:
- The old attempt row is NOT deleted — teachers can still view the
  original submission and its annotations.
- `revision_requested` is cleared on the old attempt so that if the
  teacher requests revision again on the new submission, the cycle
  repeats cleanly.
- `publish_feedback` already clears `revision_requested` on publish,
  so the flag lifecycle is: set by request_revision → cleared on
  publish OR on new attempt creation.
*/
...
BEGIN
  ...
  INTO v_existing_attempt_id, v_existing_status, v_existing_revision
  ...
  IF v_existing_attempt_id IS NOT NULL THEN
    ...
    ELSIF v_existing_revision THEN
      -- Revision was requested: create a new attempt, clear flag on old one
    ...
    ELSE
      -- Already submitted (no revision) — return the finalized attempt
