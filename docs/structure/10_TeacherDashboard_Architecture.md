...
| Grading | `src/pages/teacher/TeacherGradingPage.tsx` | Grading hierarchy + annotation workspace + submission history (v0.9.1) |
...
- `src/lib/grading.ts` — grading hierarchy, attempt listing, submission history (v0.9.1)
- `src/lib/annotations.ts` — annotation, feedback, and scoring operations
- `src/lib/notifications.ts` — notification CRUD (v0.9.0)
...
6. NotificationsPanel (v0.9.0) shows dashboard notifications with unread badges and mark-all-read. Teacher notifications link to `/teacher-grading` route.
7. Submission History (v0.9.1) — teachers can inspect previous submissions via SnapshotViewerModal in read-only mode.
