...
| `submit_attempt` | Submit response (now calls notify_teacher_of_submission internally — v0.9.1) |
| `get_student_feedback` | Retrieve published feedback |
| `get_assignment_status` | Per-item status |
| `get_notifications` | Fetch student notifications |
...
4. Displays in order:
   - Question prompt
   - Student submission (text or audio + transcript)
   - Teacher Feedback (always before scores)
   - Teacher Notes (criterion-grouped annotations with inline score badges)
   - Overall Band Card
5. Students click annotation tags to open CommentModal in read-only mode
6. Students can read text comments and play audio comments
```

Removed in v0.9.0a: Teacher Comments section and duplicate Criterion Score Cards. Scores now appear only as inline badges next to each criterion name in Teacher Notes.
...
6. NotificationsPanel (v0.9.0) shows student notifications. Links navigate to `/student-assignments` route.
7. Student notifications: `feedback_published`, `revision_requested`, `feedback_updated` (v0.9.1).
